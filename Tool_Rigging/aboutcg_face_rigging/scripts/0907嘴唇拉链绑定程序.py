...
class zipper_class:
    ...
    def con_obj(self,main_con,main_con_switch_multiplyDivide):
        ...
        pm.connectAttr(main_con+'.zipper_L',self.__zipper_range+'.valueX',f=True)
        pm.connectAttr(main_con+'.zipper_R',self.__zipper_range+'.valueY',f=True)
...
main_con = pm.PyNode("main_con")#总控制器 需要zipper_L、zipper_R、closure属性
close_obj = pm.PyNode("zipper_multiplyDivide")#需要一个0-1的outputY属性
