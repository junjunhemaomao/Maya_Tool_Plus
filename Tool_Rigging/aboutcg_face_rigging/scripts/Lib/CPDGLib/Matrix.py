...
class getWorldMatrix:
	'''
	传出对象世界位置的类
	__init__(self,**NAME)启动
	inplus(self,joint2)输入对象
	outplus(self,outWorldTranslate)输出位置
	getWorldPos(self,outWorldTranslate)
	delete(self)删除创建的节点
	'''
	def __init__(self,**NAME):
		N = ''
	...
	def inplus(self,inObj):
		pm.connectAttr(inObj+'.worldMatrix[0]',self.__decomposeMatrixWorld+'.inputMatrix',f=True)
	def outplus(self,outWorldTranslate):
		pm.connectAttr(self.__decomposeMatrixWorld+'.outputTranslate',outWorldTranslate,f=True)
