...
class HttpClient:
    ...
    def open(self, url, method="GET", timeout=TIMEOUT, headers=None):
        ...
        req = urllib.request.Request(url, method=method,
                                     headers={"User-Agent": "Maya-PolyHaven-Integration", **(headers or {})})
...
def connect_file_to_skydome(image_path):
    ...
    file_node = cmds.ls("HDRI_file", type="file")[0] if cmds.ls("HDRI_file", type="file") else cmds.shadingNode("file",
                                                                                                                asTexture=True,
                                                                                                                name="HDRI_file")
...
class ModelingToolsUI(QtWidgets.QDialog):
    ...
    def create_widgets(self):
        ...
        for color in COLOR_PRESETS:
            ...
            style = f"background-color: rgb({int(color['rgb'][0] * 255)}, {int(color['rgb'][1] * 255)}, {int(
                color['rgb'][2] * 255)});"
    ...
    def create_connections(self):
        ...
        # HDRI工具连接
        self.hdri_open_btn.clicked.connect(
    ...
    def on_download_apply(self):
        ...
        try:
            ...
            if save_path:
                ...
                QtWidgets.QMessageBox.warning(self, "HDRI Download",
                                              "Download failed\nTried URLs:\n" + "\n".join(tried))
        except Exception as e:
